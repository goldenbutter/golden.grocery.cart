    ...
    <>
      ...
      <div className="cart-panel fixed right-0 top-0 h-full w-full max-w-md bg-white z-50 flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-cream-200">
          <h2 className="font-display text-xl font-bold text-forest-800">Your Cart</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-forest-500 font-medium">Your cart is empty</p>
              <p className="text-forest-400 text-sm mt-1">Add some fresh items to get started</p>
            </div>
          ) : (
            items.map(item => (
        ...
        {items.length > 0 && (
          <div className="border-t border-cream-200 px-6 py-4 space-y-3">
            ...
            <button onClick={clearCart} className="btn-ghost w-full text-sm text-center text-red-400 hover:text-red-600">
              Clear cart
            </button>
          </div>
        )}
