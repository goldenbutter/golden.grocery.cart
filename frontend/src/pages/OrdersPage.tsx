...
export default function OrdersPage() {
  ...
  return (
    <div className="page-enter max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="font-display text-4xl font-bold text-forest-800 mb-2">My Orders</h1>
      <p className="text-forest-500 mb-8">Track and review your past orders</p>

      {successOrderId && (
        <div className="bg-green-50 border border-green-200 text-green-800 rounded-2xl px-5 py-4 mb-6 flex items-center gap-3">
          <span className="text-2xl">🎉</span>
          <div>
            <p className="font-semibold">Order #{successOrderId} placed successfully!</p>
            <p className="text-sm text-green-600">We'll get your groceries ready soon.</p>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-24">
          ...
          <p className="mt-4 text-forest-500 font-medium">No orders yet</p>
        </div>
      ) : (
        <div className="space-y-4">
          {orders.map(order => (
            <div key={order.id} className="card p-5">
              <div className="flex items-start justify-between flex-wrap gap-3 mb-4">
                <div>
                  ...
                  <p className="text-forest-400 text-sm">{new Date(order.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                </div>
